...
class BSScriptType
{
	...
	Type GetUnmangledType(void) const
	{ // 00505B70
		if(type < kType_ArraysEnd) return (Type)type;
...
class BSScriptVariable : public BSScriptType
{
	...
	union Data
	{
		...
		operator const BSFixedString&() const
		{
			return *reinterpret_cast<const BSFixedString*>(this);
		}
		operator BSScriptObject*()
		{
			return object;
		}
		operator const BSScriptObject*() const
		{
			return object;
		}
		operator BSScriptArray*()
		{
			return arr;
		}
		operator const BSScriptArray*() const
		{
			return arr;
		}

		Data& operator=(SInt32 a_i)
		{
			i = a_i;
			return *this;
		}
		Data& operator=(UInt32 a_u)
		{
			u = a_u;
			return *this;
		}
		Data& operator=(float a_f)
		{
			f = a_f;
			return *this;
		}
		Data& operator=(bool a_b)
		{
			b = a_b;
			return *this;
		}
	...
	BSScriptVariable(const BSScriptVariable& rhs) : BSScriptType()
	{ // 005398B0
		data.u = 0;
		Assign(rhs);
	}
