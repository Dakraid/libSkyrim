				...
				public IUnknown_0112197C		  // 10
{
	...
	virtual void Unk_01(BSScript::ILoader* loader)
		override; // 01 00C5B730 { classListLock.Lock(); unlinkedClassList.SetLoader(arg); classListLock.Unlock(); } - called from 008D83C8, arg=01B334F0 (vtbl 01149B98, no rtti)
	...
	virtual bool CallStaticFunction(const BSFixedString& className, const BSFixedString& fnName, IFunctionArguments* args, IStackCallbackFunctorPtr& result)
		override; // 25 00C4FA10 - "error: Unable to call static function %s on object %s due to stack creation failure or queue full, call ignored."
	virtual bool CallMemberFunction(BSScriptObjectPtr& obj, const BSFixedString& fnName, IFunctionArguments* args, IStackCallbackFunctorPtr& result)
		override; // 26 00C4FCE0 - "error: Unable to call member function %s on object %s due to stack creation failure or queue full, call ignored."
	virtual bool CallMemberFunctionFromHandle(VMHandle handle, const BSFixedString& className, const BSFixedString& fnName, IFunctionArguments* args,
											  IStackCallbackFunctorPtr& result) override;						  // 27 00C48F10
	...
	virtual void
				   IUnknown_01148030_Unk02(UInt32 arg1, UInt32 arg2, UInt32 arg3,
										   UInt32 arg4) override; // 02 00C544D0 - "Failed to load string table for internal VM save data" "VM Save file version number is too new. Expected %u or lower, found %u"
	...
	bool GetScriptVariable(Ty* pObject, const BSFixedString& className, const BSFixedString& variableName, BSScriptVariable& out)
	{
		...
		if(!policy->IsValidHandle(handle)) return false;
	...
	DEFINE_MEMBER_FN_const(FindMemberFunction, bool, 0x00C48370, const BSScriptClassPtr& classPtr, const BSFixedString& fnName, const BSFixedString& stateName, BSScriptClassPtr& foundClassPtr,
						   IFunctionPtr& foundFuncPtr);
	...
	DEFINE_MEMBER_FN(InitScriptObject, void, 0x00C4E400,
					 BSScriptObjectPtr& objPtr); // "error: Unable to call member function OnInit on object %s due to stack creation failure or queue full, call ignored."
