...
class VMArray
{
	...
	template<class ContainerT, class _value_type = typename ContainerT::value_type, class _size_type = typename ContainerT::size_type, class _iterator = typename ContainerT::iterator,
			 class _reference = typename ContainerT::reference, class _enabler = typename std::enable_if<std::is_same<value_type, _value_type>::value>::type>
	...
	VMArray& operator=(const value_type (&a_array)[SIZE])
	{
		...
		if(size != 0 && Allocate(size)) {
			for(size_type i = 0; i < size; i++) { (*this)[i] = a_array[i]; }
		}
		return *this;
	}
