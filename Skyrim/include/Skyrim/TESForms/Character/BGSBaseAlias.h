...
class BGSBaseAlias
{
	...
	enum Flag
	{
		...
		kFlag_Optional =
			1 << 1, // 0x0002 - If checked, the quest is not required to fill this in order to start. If unchecked, the quest will fail to start if the Story Manager fails to fill this alias.
		...
		kFlag_UseStoredText =
			1
			<< 14, // 0x4000 - Check this if the alias requires text replacement (for example, a book with text replacement tags, or an alias which uses text replacement in an alternate Display Name).
