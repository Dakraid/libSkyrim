					  ...
					  public IAnimationGraphManagerHolder		  // 20
{
	...
	enum
	{
		kTypeID = (UInt32)FormType::Reference
	};
	...
	virtual void					RemoveItem(RefHandle* droppedItemHandle, TESForm* akItem, UInt32 aiCount, UInt32 mode, BaseExtraList* extraList, TESObjectREFR* moveToRef, UInt32 unk7,
											   UInt32 unk8); // 004DC790 mode 0:remove 1:steal 2:trade(item ownership changes) 3:drop 4:take 5:idk - called from 0084ABE9
...
class CrosshairRefHandleHolder
{
	...
	UInt32 CrosshairRefHandle(void) const
	{
		return crosshairRefHandle;
	}
