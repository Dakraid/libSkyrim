...
class GViewport
{
	...
	GViewport(SInt32 bw, SInt32 bh, SInt32 left, SInt32 top, SInt32 w, SInt32 h, SInt32 scleft, SInt32 sctop, SInt32 scw, SInt32 sch, UInt32 flags = 0)
	{
		...
		Width		 = w;
		Height		 = h;
		Flags		 = flags | View_UseScissorRect;
		Scale = AspectRatio = 1.0f;
	...
	{
		BufferWidth	  = src.BufferWidth;
		BufferHeight  = src.BufferHeight;
		Left		  = src.Left;
		Top			  = src.Top;
		Width		  = src.Width;
		Height		  = src.Height;
		Flags		  = src.Flags;
		ScissorLeft	  = src.ScissorLeft;
		ScissorTop	  = src.ScissorTop;
		ScissorWidth  = src.ScissorWidth;
		ScissorHeight = src.ScissorHeight;
		Scale		  = src.Scale;
...
class GRenderer : public GRefCountBase<GRenderer, GStatRender_Mem>
{
	...
	class CachedData
	{
		...
		FORCE_INLINE GRenderer* GetRenderer() const
		{
			return pRenderer;
		}
		FORCE_INLINE Handle GetRendererData() const
		{
			return hData;
		}
		FORCE_INLINE void SetRendererData(Handle h)
		{
			hData = h;
		}
	...
	class CacheProvider
	{
		...
		FORCE_INLINE bool		 CanDiscardData()
		{
	...
	class Cxform
	{
		...
		{
			return M_[0][0] == x.M_[0][0] && M_[1][0] == x.M_[1][0] && M_[2][0] == x.M_[2][0] && M_[3][0] == x.M_[3][0] && M_[0][1] == x.M_[0][1] && M_[1][1] == x.M_[1][1] && M_[2][1] == x.M_[2][1] &&
				   M_[3][1] == x.M_[3][1];
		}
	...
	struct UserData
	{
		...
		UInt32		MatrixSize;
		UInt8		PropFlags;
		UserData() : pString(NULL), pFloat(NULL), pMatrix(0), MatrixSize(0), PropFlags(UD_None) {}
	};
	...
	struct BlurFilterParams
	{
		...
		BlurFilterParams(UInt32 mode, float x, float y, UInt32 n = 1, float str = 1, Cxform& cx = Cxform::Identity) : Mode(mode), BlurX(x), BlurY(y), Passes(n), Offset(0, 0), Strength(str), cxform(cx)
		{
		}
	...
	virtual void		   SetVertexData(const void* pvertices, int numVertices, VertexFormat vf, CacheProvider* pcache = 0)												= 0;
	virtual void		   SetIndexData(const void* pindices, int numIndices, IndexFormat idxf, CacheProvider* pcache = 0)													= 0;
	...
	virtual void		   FillStyleGouraud(GouraudFillType fillType, const FillTexture* ptexture0 = 0, const FillTexture* ptexture1 = 0, const FillTexture* ptexture2 = 0) = 0;
	virtual void		   DrawBitmaps(BitmapDesc* pbitmapList, int listSize, int startIndex, int count, const GTexture* pti, const Matrix& m, CacheProvider* pcache = 0)	= 0;
	virtual void		   DrawDistanceFieldBitmaps(BitmapDesc* pbitmapList, int listSize, int startIndex, int count, const GTexture* pti, const Matrix& m, const DistanceFieldParams& params,
