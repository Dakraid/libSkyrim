...
class GFxValue
{
	...
	class DisplayInfo
	{
		...
		{
			SetFlags(V_xrotation);
			XRotation = degrees;
		}
		void SetYRotation(double degrees)
		{
			SetFlags(V_yrotation);
			YRotation = degrees;
		}
		...
		double GetPerspFOV() const
		{
			return PerspFOV;
		}
		const GMatrix3D* GetPerspective3D() const
		{
			return (IsFlagSet(V_perspMatrix3D)) ? &PerspectiveMatrix3D : nullptr;
		}
		const GMatrix3D* GetView3D() const
		{
			return (IsFlagSet(V_viewMatrix3D)) ? &ViewMatrix3D : nullptr;
		}
		bool IsFlagSet(UInt32 flag) const
		{
			return 0 != (VarsSet & flag);
		}
	...
	class ObjectInterface //: public GNewOverrideBase<GFxStatMV_Other_Mem>
	{
		...
		{
			return SetTextA(pdata, ptext, ishtml);
		}
		inline bool SetText(void* pdata, const wchar_t* ptext, bool ishtml)
		{
			return SetTextW(pdata, ptext, ishtml);
		}
		...
		inline bool GotoAndPlay(void* pdata, const char* frame, bool stop)
		{
			return GotoAndPlayA(pdata, frame, stop);
		}
		inline bool GotoAndPlay(void* pdata, UInt32 frame, bool stop)
		{
			return GotoAndPlayI(pdata, frame, stop);
		}

		inline bool IsSameContext(ObjectInterface* pobjInterface)
		{
	...
	GFxValue(ValueType type) : pObjectInterface(nullptr), Type(type)
	{
		Value.pString = 0;
	}
	GFxValue(double v) : pObjectInterface(nullptr), Type(VT_Number)
	{
		Value.NValue = v;
	}
	GFxValue(bool v) : pObjectInterface(nullptr), Type(VT_Boolean)
	{
		Value.BValue = v;
	}
	GFxValue(const char* ps) : pObjectInterface(nullptr), Type(VT_String)
	{
		Value.pString = ps;
	}
	GFxValue(const wchar_t* ps) : pObjectInterface(nullptr), Type(VT_StringW)
	{
		Value.pStringW = ps;
	}
	...
	bool IsBool() const
	{
		return GetType() == VT_Boolean;
	}
	bool IsNumber() const
	{
		return GetType() == VT_Number;
	}
	bool IsString() const
	{
	...
	{
		return GetType() == VT_StringW;
	}
	bool IsObject() const
	{
		return (GetType() == VT_Object) || GetType() == VT_Array || GetType() == VT_DisplayObject;
	}
	bool IsArray() const
	{
	...
	{
		return GetType() == VT_DisplayObject;
	}
	...
	{
		return Value.pStringW;
	}

	void SetUndefined()
	{
		ChangeType(VT_Undefined);
	}
	void SetNull()
	{
		ChangeType(VT_Null);
	...
	{
		ChangeType(VT_Boolean);
		Value.BValue = v;
	}
	void SetNumber(double v)
	{
		ChangeType(VT_Number);
		Value.NValue = v;
	}
	void SetString(const char* p)
	{
		ChangeType(VT_String);
		Value.pString = p;
	}
	void SetStringW(const wchar_t* p)
	{
		ChangeType(VT_StringW);
		Value.pStringW = p;
	}

	void SetConvertBoolean()
	{
		ChangeType(VT_ConvertBoolean);
	}
	void SetConvertNumber()
	{
		ChangeType(VT_ConvertNumber);
	}
	void SetConvertString()
	{
		ChangeType(VT_ConvertString);
	}
	void SetConvertStringW()
	{
		ChangeType(VT_ConvertStringW);
	}
...
class GFxMovie : public GRefCountBase<GFxMovie, GFxStatMV_Other_Mem>
{
	...
	inline double GFxMovie::GetVariableDouble(const char* pPathToVar) const
	{
		...
		if(v.GetType() == GFxValue::VT_Number) return v.GetNumber();
		return 0.0;
	}
