...
class GMemoryHeap
{
	...
	struct HeapDesc
	{
		...
		HeapDesc(unsigned flags = 0, UInt32 minAlign = 16, UInt32 granularity = 8 * 1024, UInt32 reserve = 8 * 1024, UInt32 threshold = ~UInt32(0), UInt32 limit = 0, UInt32 heapId = 0,
				 UInt32 arena = 0)
			: Flags(flags), MinAlign(minAlign), Granularity(granularity), Reserve(reserve), Threshold(threshold), Limit(limit), HeapId(heapId), Arena(arena)
		{
		}
...
class GMemory
{
	...
	{
		return pGlobalHeap->Alloc(size, 0);
	}
	static void* Alloc(std::size_t size, std::size_t align)
	{
		return pGlobalHeap->Alloc(size, align);
	}

	static void* AllocAutoHeap(const void* p, std::size_t size)
	{
		return pGlobalHeap->AllocAutoHeap(p, size);
	}
	static void* AllocAutoHeap(const void* p, std::size_t size, std::size_t align)
	{
		return pGlobalHeap->AllocAutoHeap(p, size, align);
	}

	static void* AllocInHeap(GMemoryHeap* heap, std::size_t size)
	{
		return heap->Alloc(size, 0);
	}
	static void* AllocInHeap(GMemoryHeap* heap, std::size_t size, std::size_t align)
	{
		return heap->Alloc(size, align);
	}

	static void* Realloc(void* p, std::size_t newSize)
	{
		return pGlobalHeap->Realloc(p, newSize);
	}

	static void Free(void* p)
	{
		return pGlobalHeap->Free(p);
	}

	static GMemoryHeap* GetHeapByAddress(const void* p)
	{
