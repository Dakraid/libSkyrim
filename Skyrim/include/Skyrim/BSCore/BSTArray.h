...
class BSTArray : public _TAlloc, public BSTArrayBase
{
	...
	~BSTArray()
	{
		for(std::size_t i = 0; i < size(); ++i) { (*this)[i].~value_type(); }
	}
	...
	{
		pointer last = _last();
		for(pointer p = _head(); p < last; ++p) p->~value_type();
		_clear();
	}
	...
	void push_back(const_reference ref)
	{
		functor_type pred(this);
		SInt32		 index = _push(pred, capacity(), sizeof(value_type));
		if(index >= 0) { new(_head() + index) value_type(ref); }
	}
	...
	void emplace_back(ValTy&&... val)
	{
		functor_type pred(this);
		SInt32		 index = _push(pred, capacity(), sizeof(value_type));
		if(index >= 0) { new(_head() + index) value_type(std::forward<ValTy>(val)...); }
	}
	...
	inline iterator erase(iterator it)
	{
		if(begin() <= it && it < end()) {
			...
			if(num > 0) _move(_head(), index, next, num, sizeof(value_type));
			_pop(1);
		}
		return it;
	}
	...
	{
		pointer p = _head() + index;
		for(std::size_t i = 0; i < num; i++) { (p++)->~value_type(); }

		size_type n		  = size();
		size_type tailNum = n - (index + num);
		if(tailNum > 0) {
			if(tailNum >= num) tailNum = num;
