...
bool NiObjectNET::InsertExtraData(NiExtraData* pkExtra)
{
	...
	for(i = m_extraDataLen; i < m_extraDataCapacity; i++) { m_extraData[i] = NULL; }
...
void NiObjectNET::DeleteExtraData(UInt16 usExtraDataIndex)
{
	...
	if(m_extraData[usExtraDataIndex]) { m_extraData[usExtraDataIndex]->DecRefCount(); }

	// Shuffle downward.
	for(std::size_t i = usExtraDataIndex; i < (m_extraDataLen - 1); i++) { m_extraData[i] = m_extraData[i + 1]; }
...
bool NiObjectNET::AddExtraData(NiExtraData* extraData) // 00AB44C0
{
	...
	if(!extraData->GetName()) // auto naming
	{
		...
		if(rttiName && (strlen(rttiName) > 0)) {
			...
			if(pcSubstr > rttiName) uiStrLength = pcSubstr - rttiName;
...
SInt32 NiObjectNET::GetIndexOf(NiExtraData* extraData)
{
	...
	if(!extraData->m_pcName) return -1;
...
bool NiObjectNET::RemoveExtraData(NiExtraData* extraData)
{
	...
	if(index >= 0) { // Remove the entry
		...
		// Shift everything up
		for(UInt16 i = index; i < m_extraDataLen - 1; i++) m_extraData[i] = m_extraData[i + 1];
