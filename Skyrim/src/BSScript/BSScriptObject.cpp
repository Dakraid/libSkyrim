...
BSScriptObject::~BSScriptObject() // 00C30E90
{
	...
	if(unk0C.GetFlag()) { InterlockedDecrement(unk0C); }
}
...
void* BSScriptObject::Resolve(UInt32 typeID) const
{
	...
	if(policy->IsType(typeID, handle) && policy->IsValidHandle(handle)) result = policy->Resolve(typeID, handle);
...
SInt32 BSScriptObject::Lock(void) // 00C30720
{
	...
	while(true) {
		...
		if(lockValue & kLockBit) {
			...
			if(InterlockedCompareExchange(&refCount, lockValue | kLockBit, lockValue) == lockValue) break;
		}
	}
	...
	while(true) {
		...
		if(lockValue & kLockBit) {
			...
			if(lockValue == 1) {
				...
				if(InterlockedCompareExchange(&refCount, lockValue + 1, lockValue) == lockValue) break;
			}
		}
	}
}
	...
	while(true) {
		...
		if(lockValue & kLockBit) {
			...
			if(lockValue == 2) {
				...
				if(InterlockedCompareExchange(&refCount, lockValue - 1, lockValue) == lockValue) return lockValue - 1;
			}
		}
	}
}
