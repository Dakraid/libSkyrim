...
void BSScriptType::ToString(BSFixedString& out) const
{
	...
	switch(singleType) {
		case kType_None: typeName = "None"; break;
		case kType_Object: {
			const BSScriptClass* klass = reinterpret_cast<BSScriptClass*>(type & 0xFFFFFFFE);
			typeName				   = "None";
			if(klass) {
				const BSFixedString& name = klass->GetName();
				if(name.length() > 0) typeName = name.c_str();
			}
		} break;
		case kType_String: typeName = "String"; break;
		case kType_Int: typeName = "Int"; break;
		case kType_Float: typeName = "Float"; break;
		case kType_Bool: typeName = "Bool"; break;
		default: typeName = "Unknown"; break;
	}

	strcpy_s(buf, typeName);

	if(bIsArray) strcat_s(buf, "[]");
...
{
	if(IsString()) return (const BSFixedString&)data;
