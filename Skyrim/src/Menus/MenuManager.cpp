...
{
	if(!menuName.length()) return nullptr;

	auto it = menuTable.find(menuName);
	if(it == menuTable.end()) return nullptr;
...
void MenuManager::OpenMenu(const BSFixedString& menuName)
{
	...
	if(ui) { ui->AddMessage(menuName, UIMessage::kMessage_Open, nullptr); }
}

void MenuManager::CloseMenu(const BSFixedString& menuName)
{
	UIManager* ui = UIManager::GetSingleton();
	if(ui) { ui->AddMessage(menuName, UIMessage::kMessage_Close, nullptr); }
}
