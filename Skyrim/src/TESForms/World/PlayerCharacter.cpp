...
{
	if(actorValue - kAdvanceableSkillOffset >= 0 && actorValue < kNumAdvanceableSkills + kAdvanceableSkillOffset) return actorValue - kAdvanceableSkillOffset;
...
float PlayerSkills::GetSkillPoints(const BSFixedString& actorValue)
{
	if(data) {
		...
		if(skillId != -1) return data->levelData[skillId].points;
	}
...
void PlayerSkills::SetSkillPoints(const BSFixedString& actorValue, float points)
{
	if(data) {
		UInt32 avId	   = LookupActorValueByName(actorValue);
		SInt32 skillId = ResolveAdvanceableSkillId(avId);
		if(skillId != -1) data->levelData[skillId].points = points;
	}
}
...
SInt32 PlayerSkills::GetSkillLegendaryLevel(const BSFixedString& actorValue)
{
	if(data) {
		UInt32 avId	   = LookupActorValueByName(actorValue);
		SInt32 skillId = ResolveAdvanceableSkillId(avId);
		if(skillId != -1) return data->legendaryLevel[skillId];
	}
...
void PlayerSkills::SetSkillLegendaryLevel(const BSFixedString& actorValue, UInt32 level)
{
	if(data) {
		UInt32 avId	   = LookupActorValueByName(actorValue);
		SInt32 skillId = ResolveAdvanceableSkillId(avId);
		if(skillId != -1) data->legendaryLevel[skillId] = level;
	}
}
...
TESObjectREFR* PlayerCharacter::GetGrabbedRef() const
{
	...
	if(handle != g_invalidRefHandle) { TESObjectREFR::LookupByHandle(handle, refPtr); }
	return refPtr;
}
...
{
	SInt32 curIndex = -1;
	if(!overlayTintMasks) return nullptr;
	...
	if(overlayTintMasks->GetAt(curIndex, foundMask)) return foundMask;
