...
void UpdateRegistrationHolder::Order(UInt32 index)
{
	...
	do {
		...
		if(cmpIndex < (count - 1)) {
			...
			if(!pCur || !pSucc || pCur->schedTime > pSucc->schedTime) cmpIndex++;
		}

		Registration* pCmpReg = m_regs[cmpIndex];
		if(!pCmpReg || !pOldReg || pCmpReg->schedTime > pOldReg->schedTime) break;
	...
	if(index != startIndex) m_regs[index] = pOldReg;
}
...
UInt32 SkyrimVM::ClearInvalidRegistrations(void)
{
	...
	UInt32 count = 0;
	while(m_updateRegHolder.Remove(invalidHandle) == true) count++;
