...
class _NativeFunction : public BSScript::NF_util::NativeFunctionBase
{
	...
	{
		if(IsStatic() != std::is_same<_Base, BSScript::StaticFunctionTag>::value) return false;

		if(m_retnType != BSScript::GetTypeID<_Retn>(state)) return false;
	...
	bool ValidateParams(VMState* state)
	{
		if(m_params.data[n].type != BSScript::GetTypeID<First>(state)) return false;
...
_NativeFunction<_Base, _Retn, _Params...>* GetFunction(const char* fnName, VMState* state)
{
	...
	if(func && !func->Validate(state)) { func = nullptr; }
...
class NativeFunctionPtr
{
	...
	NativeFunctionPtr(const char* fnName) : m_ptr(nullptr)
	{
		...
		if(ptr && ptr->Validate(state)) { m_ptr = ptr; }
	}
