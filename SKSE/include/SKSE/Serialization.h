...
struct ReadInterface
{
	...
	{
		UInt16 len;
		if(!ReadRecordData(intrfc, len)) return false;
		if(len > 256) return false;

		char buf[257] = {0};
		if(len > 0) {
			if(intrfc->ReadRecordData(buf, len) != len) return false;
		}
		buf[len] = 0;
		str		 = buf;
	...
	{
		VMHandle handle;
		if(!ReadRecordData(intrfc, handle)) return false;
...
struct WriteInterface
{
	...
	static bool WriteRecordData(SKSESerializationInterface* intrfc, BSFixedString& str)
	{
		...
		if(len > 256) return false;
		if(!WriteRecordData(intrfc, len)) return false;
		if(len > 0) {
			if(!intrfc->WriteRecordData(str.c_str(), len)) return false;
		}
		return true;
	}
...
struct VMHandleInterface
{
	...
	// has handle
	template<class Ty>
	static bool HasHandle(VMHandle handle, Ty& val)
	{
		return false;
	}
	...
	// has form
	template<class Ty>
	static bool HasForm(void* obj, Ty& val)
	{
		return false;
	}
...
class Serializable : public SerializableBase, public std::forward_list<std::tuple<Params...>>
{
	...
	virtual bool Save(SKSESerializationInterface* intrfc) override
	{
		...
		if(!WriteRecordData(intrfc, size)) { return false; }
	...
	virtual bool Load(SKSESerializationInterface* intrfc, UInt32 version) override
	{
		...
		std::size_t size = 0;
		if(!ReadRecordData(intrfc, size)) { return false; }
	...
	void Register(Params... params)
	{
		...
		if(!bEmptyFound) { this->push_front(entry); }
