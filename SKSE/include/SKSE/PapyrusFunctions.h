...
template<class _Base, class _Result, class... _Params>
typename std::enable_if<BSScript::is_valid_result<_Result>::value && BSScript::is_form<_Base>::value && BSScript::are_packable<_Params...>::value>::type
	RegisterFunction(const char* className, const char* funcName, _Result (_Base::*func)(_Params...), VMState* state, UInt32 flags = 0)
{
	...
	if(flags) { state->SetFunctionFlags(className, funcName, flags); }
}

// register global function
template<class _Result, class... _Params>
typename std::enable_if<BSScript::is_valid_result<_Result>::value && BSScript::are_packable<_Params...>::value>::type RegisterFunction(const char* className, const char*	 funcName,
																																	   _Result (*func)(_Params...), VMState* state, UInt32 flags = 0)
{
...
class NativeFunction : public BSScript::NF_util::NativeFunctionBase, protected NF_helper, protected NF_caller
{
	...
	inline typename std::enable_if<(N == sizeof...(_Params))>::type SetParams(VMState* state)
	{
	}
