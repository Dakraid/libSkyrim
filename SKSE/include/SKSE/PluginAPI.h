...
struct SKSESerializationInterface
{
	...
	inline bool WriteRecord(UInt32 type, UInt32 version, const void* buf, UInt32 length) const
	{
		return m_WriteRecord(type, version, buf, length);
	}
	inline bool OpenRecord(UInt32 type, UInt32 version) const
	{
		return m_OpenRecord(type, version);
	}
	inline bool WriteRecordData(const void* buf, UInt32 length) const
	{
		return m_WriteRecordData(buf, length);
	}
