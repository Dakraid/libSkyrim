...
struct SKSEEvent
{
	...
	{
		template<class Fn>
		static inline EventResult Call(Fn f, EventT* e, SourceT* src, SinkT* snk)
		{
			return f();
		}
	};
	...
	{
		template<class Sn>
		static inline bool Equal(Sn* a, Sn* b)
		{
			return a->fn == b->fn;
		}
	};
	...
	template<class _Sink, class _Src>
	static _Src SourceType(EventResult (_Sink::*)(EventT*, _Src*));
	...
	// functor
	template<class F>
	static EventSink<F&, Arg0, CompFunctor> ObjSinkType(EventResult (F::*)());
	template<class F>
	static EventSink<F&, Arg1, CompFunctor> ObjSinkType(EventResult (F::*)(EventT*));
	template<class F>
	static EventSink<F&, Arg2, CompFunctor> ObjSinkType(EventResult (F::*)(EventT*, SourceT*));
	template<class F>
	static EventSink<F&, Arg3, CompFunctor> ObjSinkType(EventResult (F::*)(EventT*, SourceT*, SinkT*));

	// lambda
	template<class F>
	static EventSink<F, Arg0, CompLambda> ObjSinkType(EventResult (F::*)() const);
	template<class F>
	static EventSink<F, Arg1, CompLambda> ObjSinkType(EventResult (F::*)(EventT*) const);
	template<class F>
	static EventSink<F, Arg2, CompLambda> ObjSinkType(EventResult (F::*)(EventT*, SourceT*) const);
	template<class F>
	static EventSink<F, Arg3, CompLambda> ObjSinkType(EventResult (F::*)(EventT*, SourceT*, SinkT*) const);
	...
	template<class F>
	static auto SinkType(F f) -> decltype(ObjSinkType(&F::operator())){};
};
...
inline void AddEventSink(BSTEventSink<EventT>* sink)
{
	...
	if(sink && source) { source->AddEventSink(sink); }
}
