...
class dump_rtti_plugin : public SKSEPlugin
{
	...
	virtual bool OnLoad()
	{
		...
		for(UInt32 i = data_begin; i < data_end; i += sizeof(UInt32)) {
			...
			if(*p == type_info_vtbl) { SearchVTable(i); }
		}
	...
	static void PrintVirtuals(VtblList& vtblList)
	{
		...
		for(auto vtbl : vtblList) {
			...
			for(int i = 0; text_begin <= vtbl[i] && vtbl[i] < text_end; i++) {
				...
				if(numPad < 4) numPad = 4;
				for(int i = numPad; i > 0; --i) gLog << ' ';

				gLog << "// " << offset;
				if(!body.empty()) { gLog << ' ' << body; }
	...
	static void CheckCode(UInt32 addr, std::string& retOut, std::string& paramsOut, std::string& bodyOut)
	{
		...
		} else if(code[0] == 0xC2) // retn imm
		{
			...
			switch(imm) {
				case 4: params = "UInt32 arg"; break;
				case 8: params = "UInt32 arg1, UInt32 arg2"; break;
				case 12: params = "UInt32 arg1, UInt32 arg2, UInt32 arg3"; break;
				case 16: params = "UInt32 arg1, UInt32 arg2, UInt32 arg3, UInt32 arg4"; break;
				default: {
					char buf[32];
		...
		if(rtti->hierarchy) {
			...
			for(UInt32 i = 1; i < hierarchy->length; i++) {
				...
				if(node->offset == rtti->offset) {
					...
					if(it != vtblMap.cend()) return it->second.front();
				}
			}
		}
